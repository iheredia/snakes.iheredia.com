...
(function() {
  var DrawingCanvas, Food, Game, MainSnake, MirrorSnake, Snake, _currentColor, distance, equiprobable, intersect, nextColor, randomPosition, shiftPositionFarFrom,
  ...
  shiftPositionFarFrom = function(snake) {
    var shiftPosition;
  ...
  MirrorSnake = (function(superClass) {
    ...
    function MirrorSnake(params) {
      ...
      this.offset = 0;
      this.drawingCanvas = new DrawingCanvas($('canvas'));
      this.shiftPoint = shiftPositionFarFrom(this.originalSnake);
    }
    ...
    MirrorSnake.prototype.occupiedSpace = function() {
      var j, len, originalPositions, position, shiftedPositions;
      originalPositions = this.originalSnake.history.slice(this.offset, this.offset + this.length);
      shiftedPositions = [];
      for (j = 0, len = originalPositions.length; j < len; j++) {
        position = originalPositions[j];
        shiftedPositions.push({
          x: this.drawingCanvas.normalizeWidth(position.x + this.shiftPoint.x),
          y: this.drawingCanvas.normalizeHeight(position.y + this.shiftPoint.y)
        });
      }
